...
ohjelma3.c: In function ‘main’:
ohjelma3.c:11:12: warning: format ‘%lf’ expects argument of type ‘double *’, but argument 2 has type ‘double’ [-Wformat=]
   11 |   scanf("%lf", num);
      |          ~~^   ~~~
      |            |   |
      |            |   double
...
int main() {
	...
	for(;;) {
		...
		if(num < 0) {
			break; // negatiivinen numero poistuu loopista
		}
		sum += num;
		count++;
	}
